	...
	<article class="boardArticle">
		...
		<?php
			exit;
				...
				$sql = 'select b_type, b_title from board_free where b_no = ' . $bNo;
				$result = $con->query($sql);
				$row = $result->fetch_assoc();
		?>
		<div id="boardDelete">
			<form action="free_delete_update.php" method="post">
				...
				<input type="hidden" name="b_type" value="<?php echo $row['b_type']?>">
				<table>
				...
				<div class="btnSet">
					...
					<a href="./free_index.php?<?php echo $row['b_type']?>" class="btnList btn">목록</a>
				</div>
			</form>
		</div>
	<?php
